...
function Calculator() {
  const [localBrewMethod, setLocalBrewMethod] = useLocalStorage<number>(
    "brewMethod",
    0,
  );
  const [localCoffeeUnit, setLocalCoffeeUnit] = useLocalStorage<Unit>(
    "coffeeUnit",
    "g",
  );
  const [localWaterUnit, setLocalWaterUnit] = useLocalStorage<Unit>(
    "waterUnit",
    "ml",
  );
  const [method, setMethod] = useState<BrewMethod>(
    brewMethods[localBrewMethod],
  );
  const [strength, setStrength] = useState<Strength>(
    brewMethods[localBrewMethod].strengths[1],
  );
  ...
  const handleCoffeeStepper = (e: React.SyntheticEvent<EventTarget>) => {
    if (!(e.target instanceof HTMLButtonElement) || !e.target.dataset.direction)
      return;
